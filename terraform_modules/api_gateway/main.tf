...
resource "null_resource" "generate_dashboard" {
  ...
  provisioner "local-exec" {
    command = "python3 ${path.module}/../../bin.py --name ${var.grafana_configuration.name} --environment ${var.grafana_configuration.environment} ${local.notification_args} --data_source ${var.grafana_configuration.data_source} api-gateway ${local.lambda_args} | json_pp > ${local.dahboard_path}"
  }
