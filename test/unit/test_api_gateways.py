...
class TestAPIGatewayDashboards:
    def test_should_generate_proper_requests_5xx_graph(self):
        ...
        expected_targets = [
            InfluxDBTarget(
                alias="5xx",
                query='SELECT sum("5xx_error_sum") FROM "autogen"."cloudwatch_aws_lambda" WHERE ("api_name" = \'{}\') AND $timeFilter GROUP BY time(5m) fill(0)'.format(
                    apig_name
                ),
            ),
            InfluxDBTarget(
                alias="requests",
                query='SELECT sum("count_sum") FROM "autogen"."cloudwatch_aws_lambda" WHERE ("api_name" = \'{}\') AND $timeFilter GROUP BY time(5m) fill(0)'.format(
                    apig_name
                ),
            ),
        ]
